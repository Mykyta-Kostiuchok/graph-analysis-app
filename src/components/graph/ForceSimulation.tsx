...
>(
  (
    { graphData, width = 800, height = 600, onNodeClick, maxLabels = 8 },
    ref,
  ) => {
    ...
    const highlightNode = (
      ...
      // Highlight labels of connected nodes (все связанные, независимо от degree)
      svg
        .selectAll(".labels text")
        .filter((d: any) => connectedNodes.has(d.id))
        .attr("opacity", 1)
      ...
      const node = g
        ...
        .on("mouseout", function (event, d) {
          if (!selectedNodeRef.current) {
            ...
            g.selectAll(".labels text")
              .filter((l: any) => l.id === (d as any).id)
              .attr("opacity", getDefaultLabelOpacity((d as any).id));
          }
        });
